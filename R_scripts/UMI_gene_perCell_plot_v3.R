...
if (length(unique(linear$V1)) < 20000){
  ...
  if (length(cells) > step){
      ...
      for (i in 1:nrow(window)){
      	  ...
      	  Mx.sub <- as(acast(sub, V2~V1, value.var="V3", fill=0, fun.aggregate = sum), "sparseMatrix")
     ...
     Df2 <- as(acast(linear, V2~V1, value.var="V3", fill=0, fun.aggregate = sum), "sparseMatrix")
  }
}
